...
/*
 * bp.RgstrBOMUsr(); sa.assertEquals(true, bUserRegistration,
 * "sTestCaseName_1"); bp.BOMlogin(); boolean bUserLogin = bp.BOMlogin();
 * sa.assertEquals(true, bUserLogin, sTestCaseName_2); bp.BOMADMINlogin();
 * 
 * boolean bAdminLogin=bp.BOMADMINlogin(); sa.assertEquals(true, bAdminLogin,
 * sTestCaseName_3); bp.AdminAproveUser(); boolean bAdminApprove
 * =bp.AdminAproveUser(); sa.assertEquals(true, bAdminApprove, sTestCaseName_4);
 * bp.BOMlogin(); boolean bUserlogin2=bp.BOMlogin(); sa.assertEquals(true,
 * bUserlogin2, sTestCaseName_5); bp.UserRgstrByAdmn(); boolean
 * bRegistrationByAdmin =bp.UserRgstrByAdmn(); sa.assertEquals(true,
 * bRegistrationByAdmin, sTestCaseName_6); bp.UpdateRoleByAdmin(); boolean
 * bUpdateRole =bp.UpdateRoleByAdmin(); sa.assertEquals(true, bUpdateRole,
 * sTestCaseName_7); sa.assertAll(); }
 * 
 * @Test public void VerifyChangeShikeisho() throws Exception {
 * bp.BOMusrLocators(); bp.BOMlogin(); bp.SearchShikeisho();
 * bp.VerifyChangedVAlues(); bp.ChangesApproving(); bp.BOMlogout(); }
 * 
 * @Test public void AssignUserUpdtToEpics() throws Exception {
 * bp.BOMusrLocators(); bp.BOMlogin(); bp.SearchShikeisho();
 * bp.AssignBOMUserforLMAprvShi(); bp.BOMlogout();
 * 
 * }
 * 
 * @Test public void ApproveByBOM() throws Exception { login as RD user approve
 * login as LM user approve bp.BOMlogin();
 * bp.AfterRDLMviewConfirmQuantityByBOM(); bp.BOMlogout(); }
 */
